/**
 * Campaign: July 2026 "Last Call" — SEND (background)
 *
 * POST /.netlify/functions/campaign-july-last-call-send-background
 ...
 *   campaign_runs/{auto-id}  with started_at, results, total_attempted.
 *
...
exports.handler = async (event) => {
  ...
  try {
    ...
    console.warn('[campaign-july-last-call] email_log scan failed (proceeding):', e.message);
  }
  ...
  for (var i = 0; i < recipients.length; i++) {
    ...
    try {
      ...
      console.error('[campaign-july-last-call] send error for', r.id, ':', err.message);
    }
