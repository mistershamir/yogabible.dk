...
(function() {
  ...
  function loadRecordings() {
    ...
    function renderCard(item) {
      ...
      // AI Summary & Quiz accordions (bilingual: prefer _da/_en fields, fall back to legacy aiSummary/aiQuiz)
      if (item.aiStatus === 'complete') {
        var summaryLabel = isDa ? 'Resumé' : 'Summary';
        var quizLabel = isDa ? 'Test din viden' : 'Test Your Knowledge';
        var summaryContent = isDa ? (item.aiSummary_da || item.aiSummary || '') : (item.aiSummary_en || item.aiSummary || '');
        var quizRaw = isDa ? (item.aiQuiz_da || item.aiQuiz || '[]') : (item.aiQuiz_en || item.aiQuiz || '[]');
        ...
        card += '<p class="yb-ai-disclaimer">' + (isDa
          ? 'Denne opsummering er automatisk genereret af AI baseret på sessionens transskription. Den kan indeholde unøjagtigheder eller mangle vigtige detaljer.'
          : 'This summary is automatically generated by AI based on the session transcript. It may contain inaccuracies or miss important details.') + '</p>';
        card += '<div class="yb-ai-summary">' + summaryContent + '</div>';
        card += '</div></div>';
        ...
        if (quizData.length > 0) {
          ...
          card += '<p class="yb-ai-disclaimer">' + (isDa
            ? 'Denne quiz er automatisk genereret af AI og kan indeholde fejl. Brug den som et læringsværktøj, ikke som en autoritativ kilde.'
            : 'This quiz is automatically generated by AI and may contain errors. Use it as a learning tool, not as an authoritative source.') + '</p>';
