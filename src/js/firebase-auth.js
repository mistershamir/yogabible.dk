  ...
  function attemptCreateMindbodyClient(firstName, lastName, email) {
    return fetch('/.netlify/functions/mb-client', {
      ...
      if (res.status === 409) {
        ...
        return linkExistingMindbodyClient(email).then(function() { return { linked: true }; });
      }
      return res.json().then(function(data) {
        // MB POST returns client.id (lowercase via our wrapper) or client.Id (raw)
        var id = data && data.client && (data.client.id || data.client.Id);
        if (!id) {
    ...
    return attemptCreateMindbodyClient(firstName, lastName, email)
      ...
      .then(function(result) {
        ...
        if (result && result.linked) {
          // linkExistingMindbodyClient already persisted the ID
          return result;
        }
